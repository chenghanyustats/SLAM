plot_pred_gp_f_y <- function(x, y, idx = NULL, x_test, mu_test,
                             ...
                             title = NULL, is.legend = FALSE, pred_lwd = 1,
                             y_lwd = 1, plot.type = "p", pred_lty = 2,
                             t_est = NULL, x_a = 0, x_b = 2,
                             legend.loc = NULL, is.true.fcn = TRUE,
                             regfcn_type = 1, regfcn = NULL, ...) {
    plot(x, y, pch = pch, xlim = xlim, ylim = ylim, cex = cex, xlab = xlab,
         ylab = ylab, type = plot.type, lwd = y_lwd, las = 1, ...)
    if (is.true.fcn) {
        # lines(idx, regfcn(scale_back(idx, x_b - x_a, x_a)),
        #       col = "red", lwd = 2)
        if (regfcn_type == 1) {
            lines(idx, regfcn(idx, ...), col = "red", lwd = 2)
        } else if (regfcn_type == 2) {
            lines(idx, regfcn2(idx, ...), col = "red", lwd = 2)
        } else if (regfcn_type == 3) {
            lines(idx, regfcn3(idx, ...), col = "red", lwd = 2)
        } else if (regfcn_type == 4) {
            lines(idx, regfcn4(idx, ...), col = "red", lwd = 2)
        } else if (regfcn_type == 5) {
            lines(idx, regfcn5(idx, ...), col = "red", lwd = 2)
        }
    }
    # lines(idx, regfcn(idx), col = "red", lwd = 2)
    lines(x_test, mu_test, col = "blue", lwd = pred_lwd, lty = pred_lty)
    if(is.pred.f) {
    ...
    if(is.der.line) {
        abline(v = cri_pts, lwd = 1.5, col = "dark green", lty = 1)
    }
    if (is.legend) {
        if (is.pred.y) {
            if(is.der.line) {
