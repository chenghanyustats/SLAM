...
par(mfrow = c(1, 1))
par(mar = c(3.2, 3.5, 2, 0), mgp = c(2, 0.8, 0))
plot(c(x1,x2),
     c(f1(x1, t_sim_lst[[1]]$t1_1[1]), f2(x2, t_sim_lst[[1]]$t1_2[1], x1[length(x1)],
                                          t_sim_lst[[1]]$t1_1[1])),
     type = "l", col = 1, main = "Group 1", ylim = c(-2, 6), las = 1, cex.main = 2,
     xlab = "x", ylab = "f(x)", cex.lab = 2, lwd = 2)
for (i in 2:no_data) {
    lines(c(x1,x2),
         c(f1(x1, t_sim_lst[[1]]$t1_1[i]), f2(x2, t_sim_lst[[1]]$t1_2[i],
                                              x1[length(x1)],
                                              t_sim_lst[[1]]$t1_1[i])),
         col = i)
}
...
plot(c(x1,x2),
     c(f1(x1, t_sim_lst[[1]]$t2_1[1]), f2(x2, t_sim_lst[[1]]$t2_2[1],
                                         x1[length(x1)],
                                         t_sim_lst[[1]]$t2_1[1])),
     type = "l", col = 1, main = "Group 2", ylim = c(-0.5, 6), las = 1,
     xlab = "x", ylab = "f(x)", cex.main = 2, cex.lab = 2, lwd = 2)
for (i in 2:no_data) {
    lines(c(x1,x2),
          c(f1(x1, t_sim_lst[[1]]$t2_1[i]), f2(x2, t_sim_lst[[1]]$t2_2[i],
                                               x1[length(x1)],
                                               t_sim_lst[[1]]$t2_1[i])),
          col = i)
}
...
# abline(v = c(r_1_loc(r2_1), r_2_loc(r2_2)), lty = 2)
...
YY1_lst_20 <- vector("list", length = n_rep)
YY1_lst_20 <- lapply(YY1_lst_20, function(x) {
    vector(mode = "list", length = no_data)
})

YY2_lst_20 <- vector("list", length = n_rep)
YY2_lst_20 <- lapply(YY2_lst_20, function(x) {
    vector(mode = "list", length = no_data)
})
...
for (j in 1: n_rep) {
    for (k in 1:no_data) {
        ...
        y <- c(f1(x1, t_sim_lst[[j]]$t1_1[k]),
            ...
            rnorm(n, 0, sig)
        YY1_lst_20[[j]][[k]] <- list(x = xx, y = y)
        set.seed(seeds[k])
        y <- c(f1(x1, t_sim_lst[[j]]$t2_1[k]),
               f2(x2, t_sim_lst[[j]]$t2_2[k], x1[length(x1)], t_sim_lst[[j]]$t2_1[k])) +
            rnorm(n, 0, sig)
        YY2_lst_20[[j]][[k]] <- list(x = xx, y = y)
    }
}

# load("./data/data_from_model.RData", verbose = TRUE)
